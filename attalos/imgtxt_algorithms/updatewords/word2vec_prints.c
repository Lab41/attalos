...
void InitNet() {
  ...
  if (negative>0) {
    ...
    if (vector_files[0])
      ReadVectors();
    else for (a = 0; a < vocab_size; a++) for (b = 0; b < layer1_size; b++)
...
int main(int argc, char **argv) {
  ...
  printf("Checking vocabulary for %s\n", "running");
  if (read_imvocab_file[0]) {
    i = SearchImVocab("hot_tomale");
    printf("Was i in the image vocabulary? %d\n", i);
    for (i=0; i< imvocab_size; i++)
	printf("Vocabulary word is: %d - %s\n", i, imvocab[i].word);
  } 
